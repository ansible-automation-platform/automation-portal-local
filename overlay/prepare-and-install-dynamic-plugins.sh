...
# Upstream install-dynamic-plugins.py waits forever if this lock survives a killed
# installer (e.g. Ctrl+C during make dev). Safe to remove at container start.
INSTALL_LOCK="/dynamic-plugins-root/install-dynamic-plugins.lock"
if [ -f "$INSTALL_LOCK" ]; then
    echo "Removing stale $INSTALL_LOCK (prior install interrupted)"
    rm -f "$INSTALL_LOCK"
fi
